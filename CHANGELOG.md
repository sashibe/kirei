...
### ArTryOnScreen
- キラリ表示位置をメニュータブ直上（`SHEET_MIN + 8px`）に固定
- タイミングを3秒→5秒表示に変更（ミラー画面と統一）、ギャップ500ms
- ヨイショメッセージが表示されない不具合修正（hide/show 競合を `showKirariMsg()` で解消）
- `getYoishoMessage(category, lang)` をモジュール関数に昇格
- 商品・カラー選択時にもヨイショメッセージを表示（リップA→B、カテゴリ切替ともに対応）
