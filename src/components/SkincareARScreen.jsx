  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: '60px 20px 0',
        paddingBottom: 120, zIndex: 2, pointerEvents: 'none',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14, pointerEvents: 'auto' }}>
          ...
          <input type="range" min={0} max={100} value={sliderValue}
            onChange={e => { setUserInteracted(true); setSliderValue(Number(e.target.value)); }}
      ...
      <div style={{
        ...
        <button onClick={() => setWhyOpen(v => !v)} style={{
          ...
          <span style={{ transform: whyOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>{'\u25BC'}</span>
        </button>
        {whyOpen && (
          <div style={{ marginBottom: 10 }}>
            ...
            <TurnoverDiagram t={t} />
            <p style={{ fontSize: 12, color: '#64748b', margin: '6px 0', lineHeight: 1.6 }}>
...
function TurnoverDiagram({ t }) {
  return (
    <svg viewBox="0 0 280 70" style={{ width: '100%', height: 'auto', margin: '4px 0' }}>
      ...
      <text x="140" y="10" textAnchor="middle" fontSize="9" fontWeight="bold" fill="#ec4899">{t('skincare_ar.two_weeks')}</text>
      ...
      <text x="140" y="68" textAnchor="middle" fontSize="8" fill="#94a3b8">{t('skincare_ar.turnover_label')}</text>
    </svg>
  );
}
