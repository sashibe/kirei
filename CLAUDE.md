...
- [ ] スコア履歴・トレンド表示
- [ ] プッシュ通知（定期チェックリマインダー）
- [ ] 「3日後の自分」マジックミラーモード（後述）

### 構想: デュアルミラーモード

KIREIの「デジタルミラー」コンセプトを拡張し、2つのモードを提供する。

- **🪞 リアルミラー**（デフォルト）— 正直な映り。肌チェック・デンタルチェック用
- **✨ マジックミラー**（ARトライオン時）— 「スキンケアを頑張った3日後の自分」

マジックミラーの補正内容:
- 肌の平滑化（毛穴・小ジワをソフトフォーカス）
- トーンアップ（肌色を明るく補正）
- 軽い小顔補正（Face Ovalを数%内側にオフセット）

**ポジショニング**: ランコム/Sephora等の「盛りすぎ」ではなく、「3日後」という信じられる範囲の補正。リアルミラーで課題を見せ、マジックミラーでケア後の姿を見せることで、KIREI SELECTの購買を自然に促す。

キラリのセリフ:
- 「✨ これは3日後のあなた♪ 今日からケアすれば会えるよ！」
- 「🪞 リアルモードに戻すね。今の自分も素敵だよ！」

### 構想: スキンケアタブのAR化

現状のスキンケアタブ（styleTab=2）は商品リスト＋価格表示のみでAR画面をスキップしている。
ここにもARカメラを入れ、「このルーティンを3日続けた後の肌」をリアルタイムプレビューする。

**技術的なアプローチ**:
- メイクARとは別の描画パイプライン（メイクを「載せる」のではなく肌自体を「補正する」）
- Face Oval内の肌領域にビューティーフィルタを適用:
  - ソフトフォーカス（毛穴・小ジワの平滑化）→ Canvas のぼかしフィルタ or ダウンサンプル→アップサンプル
  - トーンアップ（肌の明度を上げる）→ screen ブレンドで白を薄く重ねる
  - 肌色均一化（くすみ軽減）→ 肌の平均色で overlay
  - 軽い小顔補正（Face Ovalを中心に向かって 3-5% 縮小）→ Canvas の drawImage で歪み描画
- 強度スライダーと連動（0%=リアル、100%=3日後の最大効果）
- 朝ルーティン/夜ルーティンの商品リストはカメラの下に表示

**UXフロー**:
1. スキンケアタブを選択 → カメラ起動 + ビューティーフィルタ ON
2. 「今のあなた / 3日後のあなた」スライダーで比較
3. 効果に納得 → 「結果を見る」で購入導線へ

**メイクARとの違い**:
- メイクAR: ランドマーク座標に色を「描く」（Canvas2D描画）
- スキンケアAR: 映像自体を「加工する」（ピクセル操作 or WebGLシェーダー）
- パフォーマンス要件が異なるため、WebGL移行のタイミングで実装するのが現実的
